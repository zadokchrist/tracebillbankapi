...
{
    public class VerifyCustomerController : Controller
    {
        ...
        public ActionResult Index(VerificationData data)
        {
                        ...
                        else
                        {
                            ...
                            if (authHeader != null)
                            {
                                    ...
                                    authHeaderVal.Parameter != null)
                                {
                                    ...
                                    var api = credentials[0].Trim();
                                    var password = credentials[1].Trim();
                                    string vendor = data.VendorCode.Trim();
                                    //
                                    try
                                    {
                                        ...
                                        if (isvalidconnection)
                                        {
                                            ...
                                            if (dt.Rows.Count > 0)
                                            {
                                                foreach (DataRow row in dt.Rows)
                                                {
                                                    ...
                                                    customer.Type = row["Type"].ToString();
                                        ...
                                        else
                                        {
                                            ...
                                            result = "VENDOR=" + vendor + " password=" + password + " api" + api;
                                            cust.data = customer;
                                        }
                                    }
                                    catch (Exception er)
            ...
            dh.saveRequest(data.VendorCode, JsonConvert.SerializeObject(data), cust.title,ipaddress);
            return Json(cust);
